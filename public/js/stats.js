...
const getStatsContentsFromBackend = async() => {
    loading = true
    const res = await fetch('https://quark-hardcore.com/stats_contents')
    const data = await res.json()
    loading = false
    return data
}

const addStatsContentsToDom = async() => {
    const contents = await getStatsContentsFromBackend()
